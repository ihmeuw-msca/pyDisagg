...
from pydisagg.age_split.age_var import age_id_map, match_cols, match_pats, match_pops
...
def validate_data(df, pop_df, age_id_map = age_id_map):
    ...
    if df['sex_id'].dtype == 'object':
        df['sex_id'] = df['sex_id'].apply(lambda x: x.lower() if isinstance(x, str) else x).replace({'male': 1, 'female': 2})
...
def process_data(
    ...
    if missing_columns:
        ...
        for df, cols in missing_columns.items():
            error_message += f"Dataframe {df} is missing the column(s): {', '.join(cols)}\n"
